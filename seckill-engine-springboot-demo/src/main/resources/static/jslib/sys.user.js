...
MgrUser.initColumn = function () {
    var columns = [
        ...
        {title: '性别', field: 'sexName', align: 'center', valign: 'middle', sortable: true,formatter:function (field,row,index) {
                if(row.sex==1){
                    return '男';
                }else if(row.sex==2){
                    return '女';
        ...
        {title: '状态', field: 'statusName', align: 'center', valign: 'middle', sortable: true,formatter:function (field,row,index) {
                if(row.status==1){
                    return '启用';
...
MgrUser.openAddMgr = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/sys/user/add'
    });
...
MgrUser.openChangeUser = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/sys/user/update/' + this.seItem.id
        });
        this.layerIndex = index;
    }
};
...
MgrUser.delMgrUser = function () {
    if (this.check()) {
        ...
        var operation = function(){
            ...
            var ajax = new $ax(Feng.ctxPath + "/sys/user/delete", function () {
                Feng.success("删除成功!");
...
MgrUser.freezeAccount = function () {
    if (this.check()) {
        ...
        var ajax = new $ax(Feng.ctxPath + "/sys/user/freeze", function (data) {
            Feng.success("冻结成功!");
            MgrUser.table.refresh();
        }, function (data) {
...
MgrUser.unfreeze = function () {
    if (this.check()) {
        var userId = this.seItem.id;
        var ajax = new $ax(Feng.ctxPath + "/sys/user/unfreeze", function (data) {
            Feng.success("解除冻结成功!");
            MgrUser.table.refresh();
        }, function (data) {
            Feng.error("解除冻结失败!");
...
$(function () {
    ...
    var table = new BSTable("managerTable", "/sys/user/list", defaultColunms);
    table.setPaginationType("client");
    MgrUser.table = table.init();
    var ztree = new $ZTree("deptTree", "/sys/dept/tree");
